...
export function generateMoves(board: BoardCon4): Array<MoveCon4> {
  let moves: MoveCon4[] = [];
  for (let i = 0; i < 8; i++) {
    if (board[i] === "empty") {
      moves.push(i as MoveCon4);
    }
  }
  return moves;
}
...
function generateOffset(pos: number, offset: Pos2): number | null {
  ...
  if (newPos.x > 0 && newPos.x <= 6 && newPos.y > 0 && newPos.y <= 7)
...
export function updateBoardState(
  ...
  let down = [];
  down.push(...generateOffsetLine(boardState.board, move, { x: 0, y: -1 }));
  const maxDown = down.length ? Math.max(...down.map(Number)) : null;
  if (maxDown) {
    boardState.board[maxDown] = boardState.turn;
    status = checkConnections(boardState.board, maxDown);
  } else {
    boardState.board[move] = boardState.turn;
    status = checkConnections(boardState.board, move);
  }
  const moves = generateMoves(boardState.board);
