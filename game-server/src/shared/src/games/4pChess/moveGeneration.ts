...
function CheckIsAttacked(board: Board, pos: number, color: Color) {
  ...
  for (let i = 0; i < king_pos.length; i++) {
    ...
    if (pos && AreOpposingColors(pos.color, color) && pos.type == "king")
      return true;
  }
  //check for knights
  ...
  for (let i = 0; i < moves.length; i++) {
    const pos = board[moves[i].to];
    if (pos && AreOpposingColors(pos.color, color) && pos.type == "knight")
      return true;
  }
  //check for Pawns
  ...
  for (let i = 0; i < pawn_pos.length; i++) {
    ...
    if (pos && AreOpposingColors(pos.color, color) && pos.type == "pawn")
      return true;
  }
  return false;
}
...
function checkSqare(board: Board, sq: number, color: Color): boolean {
  ...
  if (
    new_piece == null ||
    (new_piece != null && AreOpposingColors(new_piece.color, color))
  ) {
    return true;
  }
  return false;
...
function checkSqarePiece(
  ...
  const new_piece = board[sq];
  if (checkBounds(sq) == false) return false;
  if (new_piece != null && AreOpposingColors(new_piece.color, color)) {
    return true;
  }
  return false;
}
