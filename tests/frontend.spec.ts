...
test("find match and resign", async ({ browser }) => {
  ...
  await Promise.all([
    expect(page1.getByText(/white/i)).toBeVisible({ timeout: 15000 }),
    expect(page1.getByText(/black/i)).toBeVisible({ timeout: 15000 }),
    expect(page2.getByText(/white/i)).toBeVisible({ timeout: 15000 }),
    expect(page2.getByText(/black/i)).toBeVisible({ timeout: 15000 }),
  ]);
